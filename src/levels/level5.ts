import type { LevelDefinition } from "./types.js";

const level5: LevelDefinition = {
  availableComponents: ["server", "server-large", "load-balancer", "db", "db-large", "cache"],
  cacheHitRate: 0.6,
  coachMessages: [
    {
      atMs: 2_000,
      text: "You're over budget. Replace the Large Servers with Small Servers — the Load Balancer will share the load.",
    },
  ],
  componentUnlocks: [],
  feedbackText: [
    "Using smaller servers with a load balancer matched the performance of larger servers at lower cost.",
    "Choosing the right-sized components for your budget is called right-sizing your infrastructure.",
  ],
  id: "L3yW5XZioYZiysSFXxpID",
  lockedNodeIds: ["users-1", "lb-1", "cache-1", "db-1"],
  monthlyBudget: 220,
  objectiveText: "Redesign within the $220/mo budget to handle 90 ops/s.",
  solution: {
    edges: [
      { id: "edge-u-lb", source: "users-1", target: "lb-1" },
      { id: "edge-lb-s1", source: "lb-1", target: "server-1" },
      { id: "edge-lb-s2", source: "lb-1", target: "server-2" },
      { id: "edge-lb-s3", source: "lb-1", target: "server-3" },
      { id: "edge-s1-c", source: "server-1", target: "cache-1" },
      { id: "edge-s2-c", source: "server-2", target: "cache-1" },
      { id: "edge-s3-c", source: "server-3", target: "cache-1" },
      { id: "edge-c-d", source: "cache-1", target: "db-1" },
    ],
    nodes: [
      { componentType: "users", id: "users-1", position: { x: 72, y: 264 } },
      { componentType: "load-balancer", id: "lb-1", position: { x: 264, y: 264 } },
      { componentType: "server", id: "server-1", position: { x: 432, y: 72 } },
      { componentType: "server", id: "server-2", position: { x: 432, y: 264 } },
      { componentType: "server", id: "server-3", position: { x: 432, y: 432 } },
      { componentType: "cache", id: "cache-1", position: { x: 648, y: 264 } },
      { componentType: "db-large", id: "db-1", position: { x: 816, y: 264 } },
    ],
  },
  startingEdges: [
    { id: "edge-u-lb", source: "users-1", target: "lb-1" },
    { id: "edge-lb-s1", source: "lb-1", target: "server-1" },
    { id: "edge-lb-s2", source: "lb-1", target: "server-2" },
    { id: "edge-lb-s3", source: "lb-1", target: "server-3" },
    { id: "edge-s1-c", source: "server-1", target: "cache-1" },
    { id: "edge-s2-c", source: "server-2", target: "cache-1" },
    { id: "edge-s3-c", source: "server-3", target: "cache-1" },
    { id: "edge-c-d", source: "cache-1", target: "db-1" },
  ],
  startingNodes: [
    {
      componentType: "users",
      id: "users-1",
      position: { x: 72, y: 264 },
    },
    {
      componentType: "load-balancer",
      id: "lb-1",
      position: { x: 264, y: 264 },
    },
    {
      componentType: "server-large",
      id: "server-1",
      position: { x: 432, y: 72 },
    },
    {
      componentType: "server-large",
      id: "server-2",
      position: { x: 432, y: 264 },
    },
    {
      componentType: "server-large",
      id: "server-3",
      position: { x: 432, y: 432 },
    },
    {
      componentType: "cache",
      id: "cache-1",
      position: { x: 648, y: 264 },
    },
    {
      componentType: "db-large",
      id: "db-1",
      position: { x: 816, y: 264 },
    },
  ],
  timeout: 60_000,
  title: "Right-Sizing",
  trafficPeak: 0.09,
  trafficStart: 0.09,
  trafficTarget: 0.09,
  winSustainMs: 10_000,
};

export { level5 };
